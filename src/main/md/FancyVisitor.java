...
public class FancyVisitor implements MDElementVisitor {
	...
	private ArrayList<String> HTMLList = new ArrayList<String>();
	 ...
	 public void visit(Font n){
		if(tempNode.notifyNode().equals("Font")){
			...
			if(tokenList.size()<1)
			tmp = "<h"+tempNode.htype+"><font face=\"Cursive\">"+"</font></h"+tempNode.htype+">";
			else
			tmp = "<h"+tempNode.htype+"><font face=\"Cursive\">"+line+"</font></h"+tempNode.htype+">";
			line = tmp;
		}
	 }
	...
	public void tokenParser(Token t){
				...
				if(t.notifyToken().equals("<em>"))
					...
					t.setContent(t.notifyToken()+"<font color=\"blue\">");
				else if(t.notifyToken().equals("</strong>"))
					t.setContent("</font>"+t.notifyToken());
